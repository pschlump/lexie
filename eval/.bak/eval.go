...
/*
	Test: Time Consuming: Finish all the testing and get to 100% coverage on this.  Test that all errors are generated and are reasonable
		x. Test all errors 100% coverage

        *1. Computational if (40/2000=2%)                 2          85.7
        *+2. Pipe (50/2000=2.5%)                          4.5        88.2 89.5	(1.3)
        3. Assignment - rest of it (64/2000=3.2%)         7.7        91.4
----------------
        4. Test with read of JSON (33/2000=1.6%)          9.3        93
			4. add JSON -> Internal Token Format - or Handle native format
        4. Test Array ref (55/2000=2.75%)                12          95.7
        6. Prec0 Cases (27/2000=1.7%)                    14.7        97.4
	...
	Some Work:
		0. Finish Ringo -
			...
			2. template - call
			3. include
			4. all of the built-in templates
			5. pre-processing macros
...
type EvalType struct {
	...
	Lax_Type      bool         // If true, then use loose type conversions
		...
		if err_pos >= 0 && err != nil {
			...
			for ii, val := range ww {
				if ii != err_pos {
					...
					default:
						// fmt.Printf("Error (Eval001005): Can't handle type: %s as a return type from a function.", val.Type().String())
						rv.DataType = CtxType_Int
						rv.CurValue = 0
						rv.CoceLocation = com.LF(1)
						rv.LValue = false
						rv.Error = true
						rv.ErrorMsg = fmt.Sprintf("Error (Eval001005): Can't handle type: %s as a return type from a function.", val.Type().String())
						fmt.Printf("%s", rv.ErrorMsg)
					}
				}
			}
...
func x_len_e(x interface{}) (rv int) {
	...
	default:
		fmt.Printf("At: %s\n", com.LF())
		// eval.SetErrorInfo(&rv, fmt.Sprintf("Error (Eval01003): Invalid type. len() works on arrays and maps.  Type supplied %T\n", x.CurValue))
		rv = 0
	}
	return
}
...
func x_len(x interface{}) (rv int, e error) {
	...
	default:
		fmt.Printf("At: %s\n", com.LF())
		e = errors.New(fmt.Sprintf("Error (Eval01003): Invalid type. len() works on arrays and maps.  Type supplied %T\n", x))
		rv = 0
	}
	return
}
...
func x_float_type_cast(x interface{}) (rv float64, e error) {
	...
	default:
		e = errors.New(fmt.Sprintf("Error (Eval01004): Invalid type conversion, attempt to convert from %T to float\n", x))
		rv = 0
	}
	return
}
...
func x_bool_type_cast(x interface{}) (rv bool, e error) {
	...
	default:
		e = errors.New(fmt.Sprintf("Error (Eval01005): Invalid type conversion, attempt to convert from %T to bool\n", x))
	}
	fmt.Printf("At: %s\n", com.LF())
	return
}
...
func x_int_type_cast(x interface{}) (rv int, e error) {
	...
	case float64:
		rv = int(x.(float64))
	default:
		e = errors.New(fmt.Sprintf("Error (Eval01006): Invalid type conversion, attempt to convert from %T to int\n", x))
	}
	return
}
...
func (eval *EvalType) Pres0() (rv tok.Token) {
	...
	if eval.Pos < len(eval.Mm) {
			...
			if val0, t, f := eval.Ctx.GetFromContext(eval.Mm[eval.Pos].Match); f {
				...
				case CtxType_Func:
					...
					if eval.Pos < len(eval.Mm) && eval.Mm[eval.Pos].TokNo == gen.Tok_OP {
						...
						eval.SetErrorInfo(&rv, "Error (Eval00051): Function refrenced %s missing () to make call\n", match)
						return
					}
				case CtxType_Int, CtxType_Str, CtxType_Bool, CtxType_Float, CtxType_ArrayOf, CtxType_MapOf, CtxType_SMapOf, CtxType_KMapOf:
					...
					//default:
					//	eval.SetErrorInfo(&rv, "Error (Eval00052): ID Invalid type %d/%s for %s\n", t, eval.Ctx.NameOfType(t), eval.Mm[eval.Pos].Match)
					//	return
				}
			} else {
				if eval.Mm[eval.Pos].CreateId {
					...
					eval.SetErrorInfo(&rv, "Error (Eval00053): ID Missing %s\n", eval.Mm[eval.Pos].Match)
					return
				}
			}
		case gen.Tok_OP:
			...
			if eval.Pos < len(eval.Mm) && eval.Mm[eval.Pos].TokNo == gen.Tok_CL {
				...
				fmt.Printf("May be missing a close ), %s\n", com.LF())
				// xyzzy error - missing something - or wrong token
			}
		case gen.Tok_OP_SQ:
		...
		case gen.Tok_Float:
			...
			rv.DataType = eval.Mm[eval.Pos].DataType
			rv.LValue = false
		case gen.Tok_Tree_Bool:
			fmt.Printf("At: %s\n", com.LF())
			rv.CurValue = eval.Mm[eval.Pos].CurValue
			rv.DataType = eval.Mm[eval.Pos].DataType
			rv.LValue = false
		default:
			eval.SetErrorInfo(&rv, "Error (Eval00064):  Attempted to operate on token invalid token, %d\n", eval.Mm[eval.Pos].TokNo) // xyzzy display token as string
			// Add in a magic 0 or something for doing ops off of end of tokens
			rv.CurValue = 0
			rv.DataType = CtxType_Int
			return
		}
		...
		rv.DataType = CtxType_Int
		eval.SetErrorInfo(&rv, "Error (Eval00065):  Attempted to operate on token that did not exists!\n")
	}
	return
}
	...
	case gen.Tok_OP_SQ:
		...
		for eval.Pos < len(eval.Mm) {
			...
			if eval.Pos < len(eval.Mm) {
				...
				opTk = gen.Tok_CL_SQ
				// xyzzy - error - added CL to close plist!
			}
			fmt.Printf("IN ParseJSON-%s -A-: %s\n", opCLTK_s, com.LF())
		}
	...
	case gen.Tok_OP_BRACE:
		...
		for eval.Pos < len(eval.Mm) {
			...
			if opTk == gen.Tok_COMMA { //                                                   	Walk over ,
				...
				fmt.Printf("IN ParseJSON-%s -EvalThis-: %s, Pos=%d\n", opCLTK_s, com.LF(), eval.Pos)
				if eval.Pos < len(eval.Mm) {
					fmt.Printf("IN At:%s\n", com.LF())
					Tk := eval.PresG()
					if haveOne {
		...
		if TkL.DataType == CtxType_ArrayOf {
			...
			if eval.Mm[eval.Pos].TokNo == gen.Tok_COLON {
				...
				if Tk0.DataType != CtxType_Int {
					TkL = Tk0
					eval.SetErrorInfo(&TkL, "Error (Eval00071):  Attempted to index array with non-integer data.\n")
				...
				if eval.Mm[eval.Pos].TokNo == gen.Tok_COLON {
					...
					eval.SetErrorInfo(&TkL, "Error (Eval00062): invalid array reference\n")
					return
				}
			}
			// Array [ a : ]
			...
			if Tk1.DataType != CtxType_Int {
				TkL = Tk1
				eval.SetErrorInfo(&TkL, "Error (Eval00072):  Attempted to index array with non-integer data.\n")
				TkL.CurValue = 0 // Add in a magic 0 or something for doing ops off of end of tokens
				TkL.DataType = CtxType_Int
				return
			}
			if eval.Mm[eval.Pos].TokNo == gen.Tok_CL_SQ {
				...
				eval.SetErrorInfo(&TkL, "Error (Eval0063): invalid array reference\n")
				return
			}
		evalIt:
	...
	if eval.Pos < len(eval.Mm) {
		...
		if found {
			...
			default:
				...
				eval.SetErrorInfo(&TkL, "Error (Eval00031):  Attempted to use '!' operator on invalid type\n")
				return
			}
		}
	}
	...
	if eval.Pos < len(eval.Mm) {
		...
		if found {
			...
			default:
				...
				TkL.CurValue = 0
				TkL.DataType = CtxType_Int
				eval.SetErrorInfo(&TkL, "Error (Eval00030):  Attempted to use '+', '-' operator on invalid type\n")
				return
			}
		}
	}
	...
	for eval.Pos < len(eval.Mm) {
		...
		if TkL.DataType == CtxType_Int && TkR.DataType == CtxType_Int {
			...
			case gen.Tok_PCT:
				TkL.CurValue = 0
				TkL.DataType = CtxType_Int
				eval.SetErrorInfo(&TkL, "Error (Eval00001):  '%%' (modulous devision) not defined on float data.\n")
			}
		} else {
			TkL.CurValue = 0
			TkL.DataType = CtxType_Int
			eval.SetErrorInfo(&TkL, "Error (Eval00002):  Attempted to use '/', '%%', '*' operator on invalid or mixed data types\n")
		}
	}
	return
}
	...
	for eval.Pos < len(eval.Mm) {
		...
		if TkL.DataType == CtxType_Int && TkR.DataType == CtxType_Int {
			...
			case gen.Tok_MINUS:
				TkL.CurValue = 0
				TkL.DataType = CtxType_Int
				eval.SetErrorInfo(&TkL, "Error (Eval00003):  Attempted to use '-', operator on two strings - not defined.\n")
				return
			}
		} else if (TkL.DataType == CtxType_Float && TkR.DataType == CtxType_Int) || (TkL.DataType == CtxType_Int && TkR.DataType == CtxType_Float) || (TkL.DataType == CtxType_Float && TkR.DataType == CtxType_Float) {
			...
			TkL.CurValue = 0
			TkL.DataType = CtxType_Int
			eval.SetErrorInfo(&TkL, "Error (Eval0004):  Attempted to use '+', '-' operator on invalid or mixed data types, %d, %d\n", TkL.DataType, TkR.DataType)
		}
	}
	return
}
...
func (eval *EvalType) Pres5a() (TkL tok.Token) {
	...
	for eval.Pos < len(eval.Mm) {
		...
		if TkL.DataType == CtxType_Int && TkR.DataType == CtxType_Int {
			...
			TkL.CurValue = 0
			TkL.DataType = CtxType_Int
			eval.SetErrorInfo(&TkL, "Error (Eval0005):  Attempted to use '<<', '>>' operator on invalid or mixed data types\n")
		}
	}
	return
}
...
func (eval *EvalType) Pres6() (TkL tok.Token) {
	...
	for eval.Pos < len(eval.Mm) {
		...
		if TkL.DataType == CtxType_Int && TkR.DataType == CtxType_Int {
			...
			TkL.CurValue = 0
			TkL.DataType = CtxType_Int
			eval.SetErrorInfo(&TkL, "Error (Eval0006):  Attempted to use '!=', '<>', '==' operator on invalid or mixed data types\n")
		}
	}
	return
}
...
func (eval *EvalType) Pres7() (TkL tok.Token) {
	...
	for eval.Pos < len(eval.Mm) {
		...
		if TkL.DataType == CtxType_Int && TkR.DataType == CtxType_Int {
			...
			TkL.CurValue = 0
			TkL.DataType = CtxType_Int
			eval.SetErrorInfo(&TkL, "Error (Eval0007):  Attempted to use '/', '%%', '*' operator on invalid or mixed data types\n")
		}
	}
	return
}
...
func (eval *EvalType) Pres8() (TkL tok.Token) {
	...
	for eval.Pos < len(eval.Mm) {
		...
		if TkL.DataType == CtxType_Int && TkR.DataType == CtxType_Int {
			...
			TkL.CurValue = 0
			TkL.DataType = CtxType_Int
			eval.SetErrorInfo(&TkL, "Error (Eval0008):  Attempted to use '&' (bit-and),  operator on invalid or mixed data types\n")
		}
	}
	return
}
...
func (eval *EvalType) Pres9() (TkL tok.Token) {
	...
	for eval.Pos < len(eval.Mm) {
		...
		if TkL.DataType == CtxType_Int && TkR.DataType == CtxType_Int {
			...
			TkL.CurValue = 0
			TkL.DataType = CtxType_Int
			eval.SetErrorInfo(&TkL, "Error (Eval0009):  Attempted to use 'bor',  operator on invalid or mixed data types\n")
		}
	}
	return
}
...
func (eval *EvalType) PresA() (TkL tok.Token) {
	...
	for eval.Pos < len(eval.Mm) {
		...
		if TkL.DataType == CtxType_Int && TkR.DataType == CtxType_Int {
			...
			TkL.CurValue = 0
			TkL.DataType = CtxType_Int
			eval.SetErrorInfo(&TkL, "Error (Eval0010):  Attempted to use '^' (bit-xor),  operator on invalid or mixed data types\n")
		}
	}
	return
}
...
func (eval *EvalType) PresB() (TkL tok.Token) {
	...
	for eval.Pos < len(eval.Mm) {
		...
		if TkL.DataType == CtxType_Bool && TkR.DataType == CtxType_Bool {
			...
			TkL.CurValue = 0
			TkL.DataType = CtxType_Int
			eval.SetErrorInfo(&TkL, "Error (Eval0011):  Attempted to use '&&' (logical and),  operator on invalid or mixed data types\n")
		}
	}
	return
}
...
func (eval *EvalType) PresC() (TkL tok.Token) {
	...
	for eval.Pos < len(eval.Mm) {
		...
		if TkL.DataType == CtxType_Bool && TkR.DataType == CtxType_Bool {
			...
			TkL.CurValue = 0
			TkL.DataType = CtxType_Int
			eval.SetErrorInfo(&TkL, "Error (Eval0012):  Attempted to use '&&' (logical and),  operator on invalid or mixed data types\n")
		}
	}
	return
}
		...
		if eval.Pos < len(eval.Mm) {
			...
			case int:
				...
				case []interface{}, []tok.Token, map[string]interface{}, map[string]tok.Token, tok.Token, string, bool:
					if opTk == gen.Tok_EQ || opTk == gen.Tok_DCL_VAR {
						...
						TkL.CurValue = 0
						TkL.DataType = CtxType_Int
						eval.SetErrorInfo(&TkL, "Error (Eval0014):  Invalid type conversion. Type=%s\n", fmt.Sprintf("%T = %T", TkL.CurValue, TkR.CurValue))
						return
					}
				default:
					TkL.CurValue = 0
			...
			case float64:
				...
				default:
					TkL.CurValue = 0
					TkL.DataType = CtxType_Int
					eval.SetErrorInfo(&TkL, "Error (Eval0015):  Invalid type conversion. Type=%s\n", fmt.Sprintf("%T = %T", TkL.CurValue, TkR.CurValue))
					return
				}
			case string:
				switch TkR.CurValue.(type) {
				...
				default:
					TkL.CurValue = 0
					TkL.DataType = CtxType_Int
					eval.SetErrorInfo(&TkL, "Error (Eval0027):  Invalid type conversion. Type=%s\n", fmt.Sprintf("%T = %T", TkL.CurValue, TkR.CurValue))
					return
				}
			case bool:
				switch TkR.CurValue.(type) {
			...
			default:
				TkL.CurValue = 0
				TkL.DataType = CtxType_Int
				eval.SetErrorInfo(&TkL, "Error (Eval0081):  Invalid type conversion. Type=%s\n", fmt.Sprintf("%T = %T", TkL.CurValue, TkR.CurValue))
				return
			}
			if isAssign {
				switch opTk {
				...
				case gen.Tok_TILDE_EQ:
					...
					TkL.CurValue = 0
					TkL.DataType = CtxType_Int
					eval.SetErrorInfo(&TkL, "Error (Eval0017):  ~= operator not implemented yet.\n")
					return
				case gen.Tok_S_L_EQ:
					// fmt.Printf("IS = Setting [[[%s]]] to %v At: %s\n", TkL.Match, TkR.CurValue, com.LF())
					TkL.CurValue = TkL.CurValue.(int) << uint(TkR.CurValue.(int))
				...
				case gen.Tok_MOD_EQ:
					...
					TkL.CurValue = 0
					TkL.DataType = CtxType_Int
					eval.SetErrorInfo(&TkL, "Error (Eval0020):  %= operator not defined for floating point data.\n")
					return
				case gen.Tok_CAROT_EQ:
					// fmt.Printf("IS = Setting [[[%s]]] to %v At: %s\n", TkL.Match, TkR.CurValue, com.LF())
					TkL.CurValue = 0
					TkL.DataType = CtxType_Int
					eval.SetErrorInfo(&TkL, "Error (Eval0021):  ^= operator not defined for floating point data.\n")
					return
				case gen.Tok_B_OR_EQ:
					// fmt.Printf("IS = Setting [[[%s]]] to %v At: %s\n", TkL.Match, TkR.CurValue, com.LF())
					TkL.CurValue = 0
					TkL.DataType = CtxType_Int
					eval.SetErrorInfo(&TkL, "Error (Eval0022):  |= operator not defined for floating point data.\n")
					return
				case gen.Tok_B_AND_EQ:
					// fmt.Printf("IS = Setting [[[%s]]] to %v At: %s\n", TkL.Match, TkR.CurValue, com.LF())
					TkL.CurValue = 0
					TkL.DataType = CtxType_Int
					eval.SetErrorInfo(&TkL, "Error (Eval0023):  &= operator not defined for floating point data.\n")
					return
				case gen.Tok_TILDE_EQ:
					// fmt.Printf("IS = Setting [[[%s]]] to %v At: %s\n", TkL.Match, TkR.CurValue, com.LF())
					//TkL.DataType = TkR.DataType
					...
					TkL.CurValue = 0
					TkL.DataType = CtxType_Int
					eval.SetErrorInfo(&TkL, "Error (Eval0024):  ~= operator not implemented yet.\n")
					return
				case gen.Tok_S_L_EQ:
					TkL.CurValue = 0
					TkL.DataType = CtxType_Int
					eval.SetErrorInfo(&TkL, "Error (Eval0025):  <<= operator not defined for floating point data.\n")
					return
				case gen.Tok_S_R_EQ:
					TkL.CurValue = 0
					TkL.DataType = CtxType_Int
					eval.SetErrorInfo(&TkL, "Error (Eval0026):  >>= operator not defined for floating point data.\n")
					return
				}
			}
			TkL.DataType = TkR.DataType
		...
		if opTk != gen.Tok_QUEST {
			...
			if TkL.DataType == CtxType_Bool && TkL.CurValue.(bool) { // Eval True Part
				...
				if opTk == gen.Tok_COLON {
					...
					TkL.CurValue = 0
					TkL.DataType = CtxType_Int
					eval.SetErrorInfo(&TkL, "Error (Eval0094):  Missing ':' in conditional if.\n")
					return
				}
			} else {
				// fmt.Printf("At: %s\n", com.LF())
				...
				if opTk == gen.Tok_COLON {
					...
					TkL.CurValue = 0
					TkL.DataType = CtxType_Int
					eval.SetErrorInfo(&TkL, "Error (Eval0095):  Missing ':' in conditional if.\n")
					return
				}
			}
		}
					...
					if eval.Mm[eval.Pos].TokNo == gen.Tok_ID {
						...
						if /*val0*/ _, t, f := eval.Ctx.GetFromContext(eval.Mm[eval.Pos].Match); f {
							...
							default:
								...
								TkL.CurValue = 0
								TkL.DataType = CtxType_Int
								eval.SetErrorInfo(&TkL, "Error (Error00066): Function refrenced %s is not a function -- can not call it\n", match)
								return
							}
						} else {
							fmt.Printf("At: %s\n", com.LF())
							TkL.CurValue = 0
							TkL.DataType = CtxType_Int
							eval.SetErrorInfo(&TkL, "Error (Error00067): Function refrenced %s missing () to make call\n", match)
							return
						}
					} else {
						fmt.Printf("At: %s\n", com.LF())
						TkL.CurValue = 0
						TkL.DataType = CtxType_Int
						eval.SetErrorInfo(&TkL, "Error (Error00068): Token found after pipe is not an ID, found %s\n", match)
						return
					}
				}
				fmt.Printf("At: %s\n", com.LF())
...
func (eval *EvalType) PresTop() (TkL tok.Token) {
	...
	if eval.Pos < len(eval.Mm) {
		...
		TkL.CurValue = 0
		TkL.DataType = CtxType_Int
		eval.SetErrorInfo(&TkL, "Error (Eval00035): Extra tokens found at end of expressions, %s\n", eval.ListTokens())
		return
	}
	return
}

// http://ruslanspivak.com/lsbasi-part1/		Let's build an interpreter - Part 1
